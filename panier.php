...
<nav class="navbar navbar-expand-md">
 ...
 <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> 
    ...
    <a class="dropdown-item" >Vetement <i class="fa fa-shopping-bag" aria-hidden="true" style="color: red;"></i></a>
...
<?php
	if($db_found){
		$num_client='000001';
		//après modification du panier
		if(isset($_GET['id']) && isset($_GET['action'])){
			$id = $_GET['id'];
			$action = $_GET['action'];
			maj_panier($id, $num_client,$action,$db_handle);
			afficher_panier($num_client, $db_handle);
		}else{ // sans modification du panier
			afficher_panier($num_client, $db_handle);
		}
